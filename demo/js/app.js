...
以下标题使用块编辑器的 \`data-bold="false"\` 能力，字重从 Demibold-450 降为 Medium-380。

<h1 data-bold="false">H1 取消加粗（Medium-380）</h1>

<h2 data-bold="false">H2 取消加粗（Medium-380）</h2>

<h3 data-bold="false">H3 取消加粗（Medium-380）— 内部 <strong>加粗文字</strong> 应恢复 450</h3>

对比：以下是默认加粗状态的标题（Demibold-450）

# H1 默认加粗（Demibold-450）

## H2 默认加粗（Demibold-450）

### H3 默认加粗（Demibold-450）

#### 列表标题样式演示

以下列表使用块编辑器的 \`data-heading\` 能力，让列表项继承标题字号/字重/行高。

<div data-heading="h1">

- H1 无序列表项
- 第二项，bullet 应垂直居中于大字号行
...
缩进的段落内也可以包含 **加粗**、*斜体*、[链接](https://example.com) 等行内样式。
...
1. 确认设计 Token 与开发变量的映射关系
   - 字号、字重、行高
   - 间距、颜色
2. 输出标注文件，同步给研发
3. 搭建 Demo 页面，逐项走查渲染效果
