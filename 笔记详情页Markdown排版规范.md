...
| 默认宽度 | 详情页工具条添加：100%（撑满内容区）；首页导入：50%（小图模式） |
| 缩小态宽度 | 50%（用户点击功能 icon 切换） |
| 缩放方式 | 等比缩放，保持上传图片原始宽高比 |
| 圆角 | 8dp |
| 上下间距 | md (12dp) |
...
| 默认宽度 | 100%（撑满内容区） |
| 缩小态宽度 | 50%（用户点击功能 icon 切换） |
| 背景色 | 跟随涂鸦编辑页背景（Light: #FFFFFF / Dark: #000000） |
| 圆角 | 8dp |
...
- [ ] 图片（默认全屏、缩小态 50%、圆角、图注、加载失败态）
- [ ] 涂鸦卡片（默认全屏、缩小态 50%、圆角 8dp、描边 1dp、描述文字、Light/Dark 描边色值正确）
- [ ] 表格（表头、边框、对齐、窄屏横向滚动）
- [ ] 引用块（左边框 2dp、无背景、不嵌套）
- [ ] 有序列表 / 无序列表（缩进、多层级）
...
| v3.3 | 2026-05-18 | 图片与涂鸦卡片尺寸调整：默认全屏撑满内容区（移除 395dp 限制），用户可通过功能 icon 切换缩小态（50% 宽度）；涂鸦卡片背景色改为跟随编辑页背景（Light #FFFFFF / Dark #000000） |
